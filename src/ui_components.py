...
class LeaderboardComponent(BaseComponent):
    def __init__(self, x: int, right_margin: int = 260, width: int = 240):
        ...
        self.entries = [] 
        self.rects = []    
        self.selection = []  # Supports multiple selection
        self.row_height = 25
        self._tyre_textures = {}
    ...
    def draw(self, window):
        ...
        for i, (code, color, pos, progress_m) in enumerate(self.entries):
            ...
            # Highlight logic
            if code in self.selection:
                rect = arcade.XYWH((left_x + right_x)/2, (top_y + bottom_y)/2, right_x - left_x, top_y - bottom_y)
                is_primary = (code == self.selection[-1])
                bg_color = arcade.color.LIGHT_GRAY if is_primary else arcade.color.GRAY
    ...
    def on_mouse_press(self, window, x: float, y: float, button: int, modifiers: int):
        for code, left, bottom, right, top in self.rects:
            if left <= x <= right and bottom <= y <= top:
                ...
                if code in self.selection:
                    self.selection.remove(code)
                else:
                    # Allow max 2 drivers. If 2 exists, remove the oldest one.
                    if len(self.selection) >= 2:
                        self.selection.pop(0) 
                    self.selection.append(code)
                
                # Update window state
                window.selected_drivers = self.selection
                window.selected_driver = self.selection[-1] if self.selection else None
                return True
        return False
...
class DriverInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        arcade.Text(f"Driver: {code}", left + 10, header_cy, arcade.color.BLACK, 14, anchor_y="center", bold=True).draw()
        ...
        b_ratio = max(0.0, min(1.0, brake / 100.0)) if brake > 1.0 else max(0.0, min(1.0, brake))

        bar_width = 20
        bar_max_height = 80
        bar_bottom_y = bottom + 35
...
class RaceProgressBarComponent(BaseComponent):
    ...
    def __init__(self, left_margin: int = 340, right_margin: int = 260, bottom: int = 30, height: int = 24, marker_height: int = 16):
    ...
    def on_resize(self, window): self._calculate_bar_dimensions(window)
    def draw(self, window):
        ...
        if self._total_laps > 1:
            for lap in range(1, self._total_laps + 1):
                ...
                arcade.draw_line(lap_x, self.bottom + 2, lap_x, self.bottom + self.height - 2, self.COLORS["lap_marker"], 1)
                if lap == 1 or lap == self._total_laps or lap % 10 == 0:
                    arcade.Text(str(lap), lap_x, self.bottom - 4, self.COLORS["text"], 9, anchor_x="center", anchor_y="top").draw()
        ...
        arcade.draw_line(current_x, self.bottom - 2, current_x, self.bottom + self.height + 2, self.COLORS["current_position"], 3)
        if self._hover_event: self._draw_tooltip(window, self._hover_event)
    ...
    def _draw_event_marker(self, event: dict, x: float, center_y: float):
        ...
        if event_type == self.EVENT_DNF:
            ...
            arcade.draw_line(x - size, y + size, x + size, y - size, color, 2)
        elif event_type == self.EVENT_YELLOW_FLAG: self._draw_flag_segment(event, self.COLORS["yellow_flag"])
        elif event_type == self.EVENT_RED_FLAG: self._draw_flag_segment(event, self.COLORS["red_flag"])
        elif event_type == self.EVENT_SAFETY_CAR: self._draw_flag_segment(event, self.COLORS["safety_car"])
        elif event_type == self.EVENT_VSC: self._draw_flag_segment(event, self.COLORS["vsc"])
    def _draw_flag_segment(self, event: dict, color: tuple):
        ...
        segment_rect = arcade.XYWH(start_x + segment_width / 2, self.bottom + self.height + 4, segment_width, 6)
        arcade.draw_rect_filled(segment_rect, color)
    def _draw_tooltip(self, window, event: dict):
        event_type = event.get("type", "")
        label = event.get("label", "")
        lap = event.get("lap", "")
        type_names = {self.EVENT_DNF: "DNF", self.EVENT_YELLOW_FLAG: "Yellow Flag", self.EVENT_RED_FLAG: "Red Flag", self.EVENT_SAFETY_CAR: "Safety Car", self.EVENT_VSC: "Virtual SC"}
        tooltip_text = type_names.get(event_type, "Event")
        if label: tooltip_text = f"{tooltip_text}: {label}"
        if lap: tooltip_text = f"{tooltip_text} (Lap {lap})"
        event_x = self._frame_to_x(event.get("frame", 0))
    ...
    def _draw_legend(self, window):
        legend_items = [(self.COLORS["yellow_flag"], "■", "Yellow"), (self.COLORS["red_flag"], "■", "Red"), (self.COLORS["safety_car"], "■", "SC"), (self.COLORS["vsc"], "■", "VSC")]
        ...
        for i, (color, symbol, label) in enumerate(legend_items):
            x = legend_x + (i * 45)
            arcade.Text(symbol, x, legend_y + 2, color, 10, bold=True, anchor_x="center", anchor_y="center").draw()
            arcade.Text(label, x, legend_y - 10, self.COLORS["text"], 8, anchor_x="center", anchor_y="top").draw()
    def on_mouse_motion(self, window, x: float, y: float, dx: float, dy: float):
        if not self._visible: return
        self._mouse_x = x
        self._mouse_y = y
        if (self._bar_left <= x <= self._bar_left + self._bar_width and self.bottom <= y <= self.bottom + self.height + self.marker_height + 10):
    ...
    def on_mouse_press(self, window, x: float, y: float, button: int, modifiers: int) -> bool: # type: ignore
        if not self._visible: return False
        if (self._bar_left <= x <= self._bar_left + self._bar_width and self.bottom - 5 <= y <= self.bottom + self.height + 5):
...
def extract_race_events(frames: List[dict], track_statuses: List[dict], total_laps: int) -> List[dict]:
    ...
    for i in range(0, n_frames, sample_rate):
        ...
        if prev_drivers:
            ...
            for driver_code in dnf_drivers:
                ...
                events.append({"type": RaceProgressBarComponent.EVENT_DNF, "frame": i, "label": driver_code, "lap": lap})
    ...
    for status in track_statuses:
        ...
        if end_frame <= 0: continue
        if n_frames > 0: end_frame = min(end_frame, n_frames)
        event_type = None
        if status_code == "2": event_type = RaceProgressBarComponent.EVENT_YELLOW_FLAG
        elif status_code == "4": event_type = RaceProgressBarComponent.EVENT_SAFETY_CAR
        elif status_code == "5": event_type = RaceProgressBarComponent.EVENT_RED_FLAG
        elif status_code in ("6", "7"): event_type = RaceProgressBarComponent.EVENT_VSC
        if event_type: events.append({"type": event_type, "frame": start_frame, "end_frame": end_frame, "label": "", "lap": None})
    return events
...
def build_track_from_example_lap(example_lap, track_width=200):
    ...
    return (plot_x_ref, plot_y_ref, x_inner, y_inner, x_outer, y_outer, x_min, x_max, y_min, y_max)